...
消息**包含**关键词即自动回复（大小写不敏感）。规则保存在 `data/auto-reply.json`，图片下载到 `assets/auto-reply/`。

| 命令 | 说明 |
|------|------|
| `/应答设置 关键词 \| 回复文本` | 新增/覆盖规则，同条消息可附图 |
| `/应答删除 关键词` | 删除规则 |
| `/应答列表` | 查看规则 |
| `/应答帮助` | 用法说明 |

权限：`.env` 中 `AUTO_REPLY_ADMINS` 为空则全员可设置/删除；填写 openid 则仅白名单可管理。
