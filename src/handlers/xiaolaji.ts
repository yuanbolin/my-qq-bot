...
async function replyRandomImageCase(
  ...
  const imagePath =
    flag === 4 ? 'image/jiaonima.png' : `image/${imageName}`
...
export const xiaolajiHandle: GroupHandler = async (ctx) => {
  ...
  switch (msg) {
    case '小臭鸡':
      await replyRandomImageCase(ctx, 'nijibujidao.jpg', '本🐔这么乖，你不可以这样说咱哦。')
      return true
    case '小垃圾':
      await replyRandomImageCase(ctx, 'jiku.gif', '呜呜呜，你凶本🐔。')
      return true
    case '鸡憋':
      await replyRandomImageCase(ctx, 'jibie.jpg', '额 麻烦让一下下，憋不住了。')
      return true
    case '小辣鸡':
